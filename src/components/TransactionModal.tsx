...
export function TransactionModal({ open, currency, transaction, initialDate, onClose, onSave }: { open: boolean; currency: CurrencyCode; transaction?: Transaction | null; initialDate?: string | null; onClose: () => void; onSave: (transaction: Transaction) => void }) {
  ...
  return <div className="modal-backdrop ios-modal-backdrop" role="presentation"><form ref={dialogRef} onSubmit={submit} className="dialog-card ios-dialog-card max-w-2xl overflow-y-auto">
    ...
    <div className="sticky bottom-0 -mx-6 -mb-6 mt-6 flex justify-end gap-2 border-t border-slate-100 bg-white/95 px-6 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] backdrop-blur dark:border-slate-800 dark:bg-slate-900/95"><button type="button" className="secondary-btn" onClick={onClose}>Cancel</button><button className="primary-btn">{transaction ? 'Save changes' : 'Add transaction'}</button></div>
  </form></div>
}
